      ...
      {!isLoading && pageItems.length === 0 && (
        <Card>
          <EmptyState
            ...
            title={debouncedSearch ? 'No panels found' : 'No panels yet'}
            description={debouncedSearch ? `No results for "${search}"` : 'Panels are created from Client Detail — go to Clients → select a client → Add panel.'}
          />
        </Card>
      )}
    </div>
